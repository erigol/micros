...
public class UpdateCustomerDetailsController {
     ...
     */
    @Operation(summary = "Update ", description = "Method that updates a CustomerEntity with status OK 200 or EXPECTATION_FAILED 417 if exception occurs. IF not valid CustomerDetails BAD_REQUEST 400")
    @PutMapping(path = "/{userId}" /* ,consumes = {"application/json"} */)
    public ResponseEntity<CustomerDetails> update(@PathVariable(name = "userId") @NotBlank @NotNull String userId,
            @Valid @RequestBody CustomerDetails dto) {
