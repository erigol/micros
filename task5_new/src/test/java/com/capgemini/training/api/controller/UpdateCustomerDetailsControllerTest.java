...
class UpdateCustomerDetailsControllerTest {
    ...
    @Mock
    private UpdateCustomerDetailsService userService;

    @InjectMocks
    private UpdateCustomerDetailsController userController;
    ...
    public CustomerDetails createUserDto(String id) {
        return CustomerDetails.builder().customerId(id)
                // .documentType(DocumentType.valueOf("PASSPORT"))
                .documentType("PASSPORT").documentNumber("123" + id).name("john" + id).surname("green" + id)
                .lastname("junior" + id).country("ESP").telephone(123).build();
    }
